...
G_receiver_dB = 10* np.log10(np.pi**2 * D**2/Wavelength_max**2 )
Trans_eff = 0.9
G_receiver = np.pi * D**2/Wavelength_max**2 * Trans_eff
...
B_noise = 1*10**9  # The actual value needs to be computed by creating a filter!
Noise0= k*Tsys
N_tot = Noise0 * B_noise

#Loss factors
alpha= 5    # Okey, this Alpha is waaaaay too high. We need either a lot stronger amplifier or larger antenna
VSWR=1.5
Lfs_dB= -10* alpha *np.log10(4*np.pi*r/Wavelength_max) #Urban env
Lfs = 10**(Lfs_dB/10)
...
#print("PNR=", PNR)
print("SNR ratio=", SNR_1)
print("SNR db=", 20*np.log10(SNR))
